...
def plot_topo_netcdf(map,etopo_file,cmap=plt.cm.terrain,zorder=0,lonextent=(0,20),latextent=(35,60),alpha = 0.8,az = 315, alt = 45):
    ...
    map.imshow(np.array(etopo),zorder=zorder,alpha=alpha,cmap=cmap, interpolation='bilinear')
    # map.imshow(np.array(etopo_sl),zorder=zorder,alpha=alpha,cmap=cmap, interpolation='bilinear')
